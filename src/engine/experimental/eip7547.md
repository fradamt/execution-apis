...
1. Client software **MUST** check that each transaction transaction 'tx' in 'inclusionListTransactions' is valid against the pre-state of 'executionPayload', and that 'tx.max_fee_per_gas >= executionPayload.basefeePerGas + executionPayload.basefeePerGas // BASE_FEE_MAX_CHANGE_DENOMINATOR'
